...
async def main_async():
    ...
    # Nhap token - nhap tung token, done de ket thuc
    tokens = []
    print(color_rainbow("\n[NHAP TOKEN]"))
    print(color_rainbow("Nhap 'done' de ket thuc nhap token"))
    count = 1
    while True:
        token_input = log_input(f"Token {count}> ").strip()
        ...
        if token_input:
            ...
            log_success(f"Them token {count}: {token_input[:6]}...{token_input[-6:]}")
            count += 1
    
    active_tokens = tokens.copy()
    log_success(f"\nTong so token: {len(tokens)}")
    
    # Khoi dong keep online cho tung token
    ...
    # Nhap channel ID - nhap tung channel, done de ket thuc
    channel_ids = []
    channel_info = {}
    print(color_rainbow("\n[NHAP CHANNEL ID]"))
    print(color_rainbow("Nhap 'done' de ket thuc nhap channel"))
    count = 1
    while True:
        channel_id = log_input(f"Channel ID {count}> ").strip()
        ...
        if channel_id.isdigit():
            ...
            await connector.close()
            log_success(f"Them channel {count}: {channel_name} (ID: {channel_id})")
            count += 1
        else:
            log_warning("ID channel phai la so!")
    
    # Nhap cac thong so khac
    print(color_rainbow("\n[THONG SO CHAY]"))
    
    # Nhap file noi dung
    file_path = log_input("File txt chua noi dung spam: ").strip()
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            contents = [f.read().strip()]
        if not contents[0]:
            log_error("File khong co noi dung!")
            return
        log_success(f"Da doc noi dung tu file: {file_path}")
    except Exception as e:
        log_error(f"Loi doc file: {e}")
        return
    
    # Nhap delay
    delay_input = log_input("Delay giua cac tin nhan (giay) [5]: ").strip() or "5"
    try:
    ...
    if proxy_file.lower() != "skip":
        if os.path.exists(proxy_file):
            ...
            if proxy_list:
                ...
                log_warning("Khong doc duoc proxy nao!")
        else:
            log_warning(f"File {proxy_file} khong ton tai!")
    
    # Setup delay tung token
    log_info("\nSetup delay rieng cho tung token:")
    ...
    # Tao task spam cho tung token va tung channel
    max_concurrent = len(active_tokens) * len(channel_ids) * 2
    ...
    for channel_id in channel_ids:
        for token in active_tokens:
            task = asyncio.create_task(spam_task(
                semaphore, token, channel_id, channel_info[channel_id], 
                contents, base_delay, proxy_list, task_id, typing_delay
            ))
            tasks.append(task)
            task_id += 1
    ...
    log_success(f"\nKhoi dong {len(tasks)} tasks spam ({len(active_tokens)} token x {len(channel_ids)} channel)")
